...
contextBridge.exposeInMainWorld('desktop', {
  ...
  settings: {
    get: (): Promise<{ hardwareAcceleration: boolean; active: boolean; appVersion: string; noiseFilter: boolean; noiseFilterMode: 'standard' | 'advanced'; inputVolumeDb: number; inputDeviceId: string | null; outputDeviceId: string | null; outputVolume: number; participantAudioPreferences: Record<string, { volume: number; muted: boolean }>; screenShareAudioPreferences: Record<string, { volume: number; muted: boolean }> }> => ipcRenderer.invoke('settings:get'),
