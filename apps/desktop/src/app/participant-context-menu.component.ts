  ...
  template: `
    <section #menu class="participant-context-menu" role="menu" [attr.aria-label]="'Ações de ' + participantName" [style.left.px]="left" [style.top.px]="top" (pointerdown)="$event.stopPropagation()">
      ...
      @if (hasCamera || hasScreenShare) {
        <div class="media-actions" aria-label="Mídia disponível">
          @if (hasScreenShare) {
            <button class="icon-button media-button" type="button" role="menuitem" [class.active]="screenShareActive" [attr.aria-pressed]="screenShareActive" [attr.aria-label]="(screenShareActive ? 'Parar de assistir transmissão de ' : 'Assistir transmissão de ') + participantName" [title]="screenShareActive ? 'Parar de assistir transmissão' : 'Assistir transmissão'" (click)="screenShare.emit()">
              <svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="14" rx="2"/><path d="m10 9 4 2-4 2Z"/><path d="M8 22h8M12 18v4"/></svg>
            </button>
          }
          @if (hasCamera) {
            <button class="icon-button media-button" type="button" role="menuitem" [class.active]="cameraActive" [attr.aria-pressed]="cameraActive" [attr.aria-label]="(cameraActive ? 'Parar de ver câmera de ' : 'Ver câmera de ') + participantName" [title]="cameraActive ? 'Parar de ver câmera' : 'Ver câmera'" (click)="camera.emit()">
              <svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="6" width="12" height="12" rx="2"/><path d="m15 10 4.5-2.25A1 1 0 0 1 21 8.65v6.7a1 1 0 0 1-1.5.9L15 14"/></svg>
            </button>
          }
        </div>
      }
