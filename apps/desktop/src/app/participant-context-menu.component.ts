    ...
    <section #menu class="participant-context-menu" role="menu" [attr.aria-label]="'Ações de ' + participantName" [style.left.px]="left" [style.top.px]="top" (pointerdown)="$event.stopPropagation()">
      ...
      <div class="audio-section">
        ...
        <div class="volume-row">
          ...
          <input type="range" min="0" max="100" step="1" [value]="audioPreference.volume * 100" [attr.aria-label]="'Volume da voz de ' + participantName" (keydown)="onVolumeKeydown($event)" (input)="volumeChange.emit(+$any($event.target).value)">
      ...
      @if (hasScreenShareAudio) {
        <div class="audio-section">
          ...
          <div class="volume-row">
            <button class="icon-button" type="button" role="menuitem" [class.active]="screenShareAudioPreference.muted" [attr.aria-label]="screenShareAudioPreference.muted ? 'Ouvir transmissão de ' + participantName + ' novamente' : 'Silenciar transmissão de ' + participantName + ' para mim'" [title]="screenShareAudioPreference.muted ? 'Ouvir transmissão novamente' : 'Silenciar transmissão para mim'" (click)="screenShareMuteToggle.emit()">
              @if (screenShareAudioPreference.muted) { <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 5 6 9H2v6h4l5 4Z"/><path d="m16 9 5 5M21 9l-5 5"/></svg> }
              @else { <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 5 6 9H2v6h4l5 4Z"/><path d="M15.5 8.5a5 5 0 0 1 0 7"/></svg> }
            </button>
            <input type="range" min="0" max="100" step="1" [value]="screenShareAudioPreference.volume * 100" [attr.aria-label]="'Volume da transmissão de ' + participantName" (keydown)="onVolumeKeydown($event)" (input)="screenShareVolumeChange.emit(+$any($event.target).value)">
            <strong>{{ (screenShareAudioPreference.volume * 100).toFixed(0) }}%</strong>
          </div>
        </div>
      }
      ...
      <small>Volumes somente para você</small>
    </section>`,
  styles: [`
...
export class ParticipantContextMenuComponent implements AfterViewInit {
  ...
  @Input() screenShareAudioPreference: ParticipantAudioPreference = { volume: 1, muted: false };
  @Input() hasScreenShareAudio = false;
  ...
  @Output() readonly screenShareVolumeChange = new EventEmitter<number>(); @Output() readonly screenShareMuteToggle = new EventEmitter<void>();
