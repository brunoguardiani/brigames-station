...
func (service *Service) Register(ctx context.Context, username, email, password string) (User, error) {
	...
	var user User
	err = service.pool.QueryRow(ctx, "INSERT INTO users (username, email, password_hash, role_id) SELECT $1, $2, $3, id FROM roles WHERE key = 'member' RETURNING id, username, email, (SELECT key FROM roles WHERE roles.id = users.role_id), avatar_id, status", username, email, hash).Scan(&user.ID, &user.Username, &user.Email, &user.Role, &user.AvatarID, &user.Status)
	if err == nil {
		return user, nil
	}
