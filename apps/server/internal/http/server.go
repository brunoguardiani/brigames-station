...
func NewHandler(pool *pgxpool.Pool, identityService *identity.Service, tokenManager *auth.TokenManager, serverService *servers.Service, messageService *messages.Service, inviteService *invites.Service, hub *realtime.Hub) http.Handler {
	router := gin.New()
	...
	router.GET("/ws", gin.WrapF(realtimeHandler(hub, tokenManager)))
