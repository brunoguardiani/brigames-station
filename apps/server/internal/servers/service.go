...
func (service *Service) ListMembers(ctx context.Context, userID, serverID int64) ([]Member, error) {
	...
	rows, err := service.pool.Query(ctx, "SELECT users.id, users.username, server_memberships.role, users.avatar_id, users.status FROM server_memberships JOIN users ON users.id = server_memberships.user_id WHERE server_memberships.server_id = $1 ORDER BY users.username, users.id", serverID)
	if err != nil {
	...
	for rows.Next() {
		var item Member
		if err := rows.Scan(&item.ID, &item.Username, &item.Role, &item.AvatarID, &item.Status); err != nil {
